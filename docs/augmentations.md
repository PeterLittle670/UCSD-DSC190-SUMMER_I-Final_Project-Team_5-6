...
Donkeycar already supported a couple of training-time image augmentations (`BRIGHTNESS`, `BLUR`). This fork expands that list with four more, aimed specifically at the lighting conditions a small outdoor/indoor track actually produces:
...
### 1.4 The six available augmentations
...
Each generated image shows six panels: Original, Brightness, Gamma, Shadow, Noise, and an "All Conditions" combined view — useful for judging whether stacking every effect at once still leaves the lane markings visible, before you spend an hour training on it.
...
- **Calibration and `TRANSFORMATIONS` don't currently interact.** If your model uses `TRANSFORMATIONS` (e.g. `CROP`), remember that transform applies at both training and inference — but the XAI toolkit's calibration replay (see [xai.md](xai.md)) does not currently re-apply `TRANSFORMATIONS` when it replays a tub to build baselines. This is a known, unfixed gap (out of scope for this feature) — it means calibration baselines can be measured against un-cropped frames while the live model only ever sees cropped ones. Something to be aware of if you combine `TRANSFORMATIONS` with the XAI toolkit's confidence/novelty scoring.
