...
| Model path | e.g. `models/mypilot.h5` (autocompletes from `models/`) — once selected, a hint below the field shows what pipeline this model was actually trained with (e.g. `Trained with: CROP, LANE_ISOLATE (kernel=9, color_order=rgb, chroma_angle=90)`), read straight from the model's own `.preprocessing.json` sidecar (§2.9) |
...
- *Just the camera*
- *What the model actually sees (transformed)* — the frame **after** `TRANSFORMATIONS`/`POST_TRANSFORMATIONS` (crop, lane isolation, colour conversion). This is the actual input every heat map above was computed against; only shown when the pipeline changes something (a no-op pipeline would make this identical to "Just the camera")
...
- **A mismatched `TRANSFORMATIONS`/`POST_TRANSFORMATIONS` config now stops things, rather than silently misbehaving.** Training writes a `<model>.preprocessing.json` sidecar recording the exact pipeline it used; both live driving and offline analysis (§1.6/§1.7) read it back and refuse to proceed if the current config doesn't match. One gap: the plain `mc_calibrate` CLI (§1.3) does *not* run this check itself — it's enforced at drive-time and analysis-time, not at calibration-time — so it's still on you to point `--tub`/`--config` at the right pair when calibrating by hand.
...
**Two families of transforms behave differently here, and it matters which one `CROP` currently is.** `TRAPEZE` still masks: it blanks out pixels outside the region of interest but leaves the image at its original dimensions. `CROP` used to work the same way, but no longer does — it now physically deletes the excluded rows/columns and resizes what's left back to `IMAGE_W`/`IMAGE_H` (`ImgCropResize`, not the older `ImgCropMask`), so the model input keeps the same *shape* as before while actually showing a zoomed-in view of a different, smaller region of the raw frame. A masking transform mismatch used to be the dangerous case — same dimensions, no shape error, just silently wrong thresholds — but that risk is now caught directly: training saves a `<model>.preprocessing.json` sidecar recording the exact `TRANSFORMATIONS`/`POST_TRANSFORMATIONS`/`ROI_CROP_*` (and, if used, `LANE_ISOLATE_*`) pipeline it was trained with, and both live driving and offline analysis refuse to run if the current config doesn't match it (§1.9). A calibration also records which transformations built it and the dashboard raises a "Calibration stale" warning if that's drifted, independent of this check.

In the offline viewer, what you *see* by default is still the raw camera frame, with heat maps projected back onto it — for `TRAPEZE`-style masking this is a direct, no-op mapping, but for `CROP`'s crop-and-resize the toolkit now explicitly inverts the crop geometry so each hot spot lands back on the right pixels of the raw frame rather than being stretched across the whole thing. You can also switch to the *"What the model actually sees (transformed)"* layer to see the zoomed, transformed frame directly, and tick **"draw on model input"** to see the heat maps drawn on it with no projection at all — useful for sanity-checking that the projection is landing where you'd expect (§1.6).

One more transformation worth knowing about: `LANE_ISOLATE` (`ImgLaneIsolate`) is a newer, more aggressive transformation that reduces the frame to just its lane markings, largely independent of scene brightness — it exploits the fact that a lighting change is *multiplicative* (scales the whole frame by some gain), so dividing by a local estimate of the background cancels that gain out, unlike a simple edge filter's subtraction. It outputs 3 encoded channels (a bright-marking-width "tape" ratio, a colour-projected "centre line" channel, and a coarse local-contrast channel) rather than a normal-looking image — genuinely different from what a human would call "the picture," but still a valid 3-channel input the model trains on normally. It's meant to run in `POST_TRANSFORMATIONS`, after `CROP`, since without a crop first it treats window frames and door edges above the horizon as more lane tape. Two of its settings (`LANE_ISOLATE_COLOR_ORDER`, `LANE_ISOLATE_CHROMA_ANGLE`) depend on your specific camera and marking colour and fail *silently* (the centre-line channel just goes blank, no error) if wrong — preview a frame after changing either.
